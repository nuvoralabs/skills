#!/usr/bin/env bash
#
# Fetch Nuvora Nexus Relay documentation as clean Markdown.
#
# Every page on relay.nuvoralabs.com is available as Markdown by appending `.md`
# to its path. This wraps that with a 6-hour cache so repeated reads in one
...
#   fetch-docs.sh full                   The entire corpus (~830 KB — last resort)
#   fetch-docs.sh clean                  Drop the cache
#
# Paths are forgiving: `docs/outbox`, `/docs/outbox/`, `docs/outbox.md` and the
# full URL all resolve to the same page.
#
# Examples:
#   fetch-docs.sh get docs/event-sourcing articles/outbox
#   fetch-docs.sh search projection rebuild
#   fetch-docs.sh api Nuvora.Nexus.Relay.Sagas

set -euo pipefail

BASE_URL="https://relay.nuvoralabs.com"
CACHE_DIR="${TMPDIR:-/tmp}/nuvora-relay-docs"
...
case "$cmd" in
  ...
  api)
    ...
    # Nuvora.Nexus.Relay.Sagas -> api/nuvora-nexus-relay-sagas
  ...
  full)
    echo "warning: llms-full.txt is ~830 KB; prefer 'get' on specific pages." >&2
    fetch "llms-full.txt"
    ;;
